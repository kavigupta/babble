//! Library-learning runner for egg-stitch's molecule benchmark.
//!
//! Reads a corpus of molecule s-expressions (one per line, the `.bab` format),
//! optionally applies the symmetry domain-specific rewrites, runs babble's
//! beam-search lib learning, and dumps the original/rewritten programs plus the
//! learned abstractions as JSON. Modelled on the `drawings` binary, minus the
//! drawing-specific eval/SVG paths.
//!
...
struct Opts {
    ...
    #[clap(long, default_value = "harness/data_gen/res_molecules.csv")]
    output: String,
...
fn main() {
    ...
    let mut prog: Vec<Expr<Molecule>> = Program::parse(&input)
...
/// abstractions as JSON (mirrors the `drawings` binary's dump format).
fn run_with_json_dump(
    prog: Vec<Expr<Molecule>>,
    dsrs: Vec<Rewrite<AstNode<Molecule>, PartialLibCost>>,
    opts: &Opts,
) {
    let beam = opts.beams;
    ...
    let final_recexpr: RecExpr<AstNode<Molecule>> = result.final_expr.clone().into();
    ...
    let original: Vec<String> = prog
        .iter()
        .map(|e| RecExpr::<AstNode<Molecule>>::from(e.clone()).to_string())
        .collect();
    let rewritten: Vec<String> = rewritten_exprs
        .into_iter()
        .map(|e| RecExpr::<AstNode<Molecule>>::from(e).to_string())
        .collect();
    ...
    let abstractions: Vec<_> = sorted_libs
        .into_iter()
        .map(|(id, body_nodes)| {
            let r: RecExpr<AstNode<Molecule>> = body_nodes.into();
